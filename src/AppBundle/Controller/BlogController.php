...
class BlogController extends Controller
{
    ...
    /**
     * @Route("/suggest", name="blog_suggest")
     * @Method("GET")
     */
    public function autoSuggest(Request $request){
        ...
        $matchQuery->setFieldQuery('title',$search);
        $matchQuery->setFieldOperator('title','and');

        $postsPage = $finder->findPaginated($matchQuery);
        $postsPage->setMaxPerPage(10);
        $postsPage->setCurrentPage(1);

        $posts = $postsPage->getCurrentPageResults();
        $data = [];

        /** @var Post $post */
        foreach($posts as $post){
            $data[] = ['value' => $post->getTitle()];
        }
    ...
    public function searchAction(Request $request){
        ...
        /** @var PaginatedFinderInterface $finder */
        $finder = $this->container->get('fos_elastica.finder.app.post');
        $posts = $finder->findPaginated($search);
